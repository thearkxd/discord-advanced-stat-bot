...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (["ver", "give"].includes(args[0])) {
      if (!message.member.hasPermission(8)) return message.channel.error(message, "Yeterli yetkin bulunmuyor!");
      ...
      if (["şema", "schema"].includes(args[2])) {
        ...
        if (role) {
          ...
          message.channel.send(embed.setDescription(`${message.mentions.roles.first().toString()} rolüne sahip olan tüm üyelere başarıyla ${id} ID'li şemadan görevi verildi! \nGörev verilen üyeler: ${members.map((x) => x.toString()).join(", ")}`));
        } else {
          await member.giveTask(message.guild.id, schema.type, schema.count, schema.prizeCount, true, schema.duration, schema.channels.length ? channels.map((x) => x.id) : null);
          message.channel.send(embed.setDescription(`${member.toString()} üyesine başarıyla ${id} ID'li şemadan verildi!`));
        }
      }
      ...
      message.channel.send(embed.setDescription(`${id} ID'li görev başarıyla silindi!`));
      ...
      if (["ekle", "add"].includes(args[1])) {
        ...
        message.channel.send(embed.setDescription(`${id} ID'li görev şeması başarıyla silindi!`));
