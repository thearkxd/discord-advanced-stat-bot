...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    } else if (["liste", "list"].includes(args[0])) message.channel.send(embed.setDescription(global.rankdb.get("ranks").map((x) => `${Array.isArray(x.role) ? x.role.map(x => `<@&${x}>`).join(", ") : `<@&${x.role}>`}: ${x.coin}`)));
  }
};
